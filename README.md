...
The real problem, though, is that the *"de facto"* vector graphics standard nowadays is undoubtfully SVG, and that this comes with all intrinsic SVG problems, notably of being so complex that it discourages people to use it explicitly. The format is being relegated to be managed by specific software, a realm still dominated by proprietary technology or extremely complex, bulky and cumbersome open source solutions. The problem of proprietary technology is that it obviously can't be reused in programming contexts. The problem with bulky and cumbersome open source solutions is that they also inhibit software reuse in contexts different than it is currently used.
